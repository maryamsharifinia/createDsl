...
class CustomExampleDSLCodeGenerator:
    ...
    def push_rows(self):
        self.operand_stack.append("rows__")

    def push_query(self):
        self.operand_stack.append("query__")
    ...
    def is_columns(self, item):
        if item == "columns__":
            return True
        return False

    def is_rows(self, item):
        if item == "rows__":
            return True
        return False

    def is_query(self, item):
        if item == "query__":
            return True
        return False

    def is_assign(self, item):
        if item == "assign__":
            return True
        return False
    ...
    def add_columns(self):
        ...
        if self.is_to_called(temp_or_col):
            ...
            to_code = f"['addCol{self.addCol_cnt}']"
            self.addCol_cnt += 1
    ...
    def columns(self):
        ...
        code_string = ''
        is_num = False
        if self.is_query(temp_or_query):
            is_num = True
            code_string = f'{self.code_stack.pop()}'
        else:
            if temp_or_query.isnumeric():
                is_num = True
            cols = [temp_or_query]
            while len(self.operand_stack) > 0:
                temp = self.operand_stack.pop()
                if self.is_rows(temp) or self.is_assign(temp):
                    self.operand_stack.append(temp)
                    break
                cols.append(temp)
            code_string = '[' + ', '.join(cols[::-1]) + ']'
    ...
    def query(self):
        ...
        while len(self.operand_stack) > 0:
            ...
            if temp == 'step__':
                ...
                from_code = f'{self.operand_stack.pop()}'
            elif temp == 'to__':
                to_code = f'{self.operand_stack.pop()}'

        query_code = ':'.join([from_code, to_code, step_code])
    ...
    def select(self):
        ...
        if len(self.operand_stack) == 0:
            if self.is_as_called(temp_or_targetvar):
                ...
                self.code_stack.append(code_string + f" = {temp_or_targetvar}.copy()" + '\n')
            return
        else:
            while len(self.operand_stack) > 0:
                temp = self.operand_stack.pop()
        ...
        if has_assign:
            if self.is_as_called(temp_or_targetvar):
                ...
                code_string += f'\n{as_code}.iloc[{(row_code if has_row else ":")}, {(col_code if has_col else ":")}] = {assign_val}\n'
            else:
                code_string += f'.iloc[{(row_code if has_row else ":")}, {(col_code if has_col else ":")}] = {assign_val}\n'
        else:
